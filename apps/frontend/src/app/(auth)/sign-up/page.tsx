...
import { Orbit, Mail, Lock, User, ArrowRight, Eye, EyeOff } from "lucide-react";
...
export default function SignUpPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute top-1/3 -right-32 w-96 h-96 bg-tertiary/10 rounded-full blur-[120px]" />
      <div className="absolute bottom-1/3 -left-32 w-96 h-96 bg-primary/10 rounded-full blur-[120px]" />
      ...
      >
        <div className="glass-card-static p-8 md:p-10">
          ...
          <h1 className="text-2xl font-bold text-on-surface text-center mb-2">Create your account</h1>
          <p className="text-on-surface-muted text-center mb-8">Join thousands of teams collaborating in virtual spaces</p>
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-sm"><span className="bg-surface-high px-3 text-on-surface-muted">or continue with</span></div>
          </div>

          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1.5">Full Name</label>
              <Input type="text" placeholder="Dale Cooper" icon={<User className="w-4 h-4" />} />
            </div>
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1.5">Email</label>
              <Input type="email" placeholder="dale@orbithq.com" icon={<Mail className="w-4 h-4" />} />
            </div>
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1.5">Password</label>
              <div className="relative">
                <Input type={showPassword ? "text" : "password"} placeholder="Minimum 8 characters" icon={<Lock className="w-4 h-4" />} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-muted hover:text-on-surface transition-colors cursor-pointer">
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 mt-0.5 rounded border-outline-variant bg-surface-low accent-primary" />
              <span className="text-sm text-on-surface-muted">I agree to the Terms of Service and Privacy Policy</span>
            </label>
          ...
          <p className="text-center text-sm text-on-surface-muted mt-6">
            ...
            <Link href="/sign-in" className="text-primary hover:text-primary-light transition-colors">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
